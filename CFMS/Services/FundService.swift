...
class FundService: ObservableObject {
    ...
    init() {
        cacheQueue.async {
            Task { @MainActor in
                await self.loadCacheFromUserDefaults()
            }
        }
    }
    ...
    private func loadCacheFromUserDefaults() async {
        if let savedData = UserDefaults.standard.data(forKey: userDefaultsKey) {
            do {
                ...
                await addLog("主缓存已从本地加载。", type: .cache)
            } catch {
                await addLog("加载主缓存失败: \(error.localizedDescription)", type: .error)
            }
        } else {
            await addLog("UserDefaults 中没有找到主缓存数据。", type: .info)
        }
    }
    ...
    func fetchFundInfo(code: String, useOnlyEastmoney: Bool = false) async -> FundHolding {
        ...
        if let cachedData = getFromCache(code: code) {
            ...
            if isSameNavDay && isCacheFresh {
                await addLog("基金代码 \(code): 从主缓存中获取数据，净值日期为今日且缓存未过期。", type: .cache)
                return cachedData.holding
            } else {
                await addLog("基金代码 \(code): 主缓存数据净值日期非今日或缓存已过期，尝试更新。", type: .cache)
            }
        }
    ...
    private func fetchFromEastmoney(code: String) async -> FundHolding {
        ...
        guard let url1 = URL(string: urlString1) else {
            await addLog("基金代码 \(code): 天天基金API URL无效", type: .error)
            return FundHolding.invalid(fundCode: code)
        }
        
        do {
            let request1 = createURLRequest(with: url1)
            ...
            guard let httpResponse1 = response1 as? HTTPURLResponse, httpResponse1.statusCode == 200 else {
                await addLog("基金代码 \(code): 天天基金API响应状态码非200", type: .error)
                return FundHolding.invalid(fundCode: code)
            }
            ...
            guard let url2 = URL(string: urlString2) else {
                await addLog("基金代码 \(code): 天天基金详情API URL无效", type: .error)
                return holding
            }
            
            let request2 = createURLRequest(with: url2)
            ...
            guard let httpResponse2 = response2 as? HTTPURLResponse, httpResponse2.statusCode == 200 else {
                await addLog("基金代码 \(code): 天天基金详情API响应状态码非200", type: .error)
                return holding
            }
            
            guard let jsString = String(data: data2, encoding: .utf8) else {
                await addLog("基金代码 \(code): 天天基金详情API数据编码失败", type: .error)
                return holding
            }
            ...
            if let latestDate = latestNavDate, let latestNav = latestNavValue,
               let firstDate = firstInterfaceDate {
                ...
                if latestDate > firstDate {
                    ...
                    await addLog("基金代码 \(code): 使用详情API的最新净值数据（日期: \(latestDate), 净值: \(latestNav))", type: .success)
                } else {
                    await addLog("基金代码 \(code): 使用主API的单位净值数据（日期: \(firstDate), 净值: \(holding.currentNav))", type: .success)
                }
            } else if let latestDate = latestNavDate, let latestNav = latestNavValue {
                holding.navDate = latestDate
                holding.currentNav = latestNav
                await addLog("基金代码 \(code): 使用详情API的净值数据（日期: \(latestDate), 净值: \(latestNav))", type: .success)
            }
            
            holding.isValid = holding.fundName != "N/A" && holding.currentNav > 0
            
            if holding.isValid {
                await addLog("基金代码 \(code): 天天基金API解析成功", type: .success)
            } else {
                await addLog("基金代码 \(code): 天天基金API数据无效", type: .error)
            }
            
            return holding
        } catch {
            await addLog("基金代码 \(code): 天天基金API请求失败: \(error.localizedDescription)", type: .error)
            return FundHolding.invalid(fundCode: code)
        }
    }
    ...
    private func fetchReturnsFromEastmoney(code: String) async -> (navReturn1m: Double?, navReturn3m: Double?, navReturn6m: Double?, navReturn1y: Double?) {
        ...
        guard let url = URL(string: urlString) else {
            await addLog("基金代码 \(code): 天天基金收益率API URL无效", type: .error)
        ...
        do {
            ...
            guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
                await addLog("基金代码 \(code): 天天基金收益率API响应状态码非200", type: .error)
                return (nil, nil, nil, nil)
            }
            
            guard let jsString = String(data: data, encoding: .utf8) else {
                await addLog("基金代码 \(code): 天天基金收益率API数据编码失败", type: .error)
                return (nil, nil, nil, nil)
            }
            ...
            await addLog("基金代码 \(code): 收益率数据解析完成: 1月=\(navReturn1m ?? 0), 3月=\(navReturn3m ?? 0), 6月=\(navReturn6m ?? 0), 1年=\(navReturn1y ?? 0)", type: .success)
            return (navReturn1m, navReturn3m, navReturn6m, navReturn1y)
            
        } catch {
            await addLog("基金代码 \(code): 天天基金收益率API请求或正则解析失败: \(error.localizedDescription)", type: .error)
            return (nil, nil, nil, nil)
        }
    }
    ...
    private func fetchFromTencent(code: String) async -> FundHolding {
        ...
        guard let url = URL(string: urlString) else {
            await addLog("基金代码 \(code): 腾讯财经API URL无效", type: .error)
            return FundHolding.invalid(fundCode: code)
        }
        ...
        do {
            ...
            let (data, response) = try await URLSession.shared.data(for: request)
            guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
                await addLog("基金代码 \(code): 腾讯财经API响应状态码非200", type: .error)
                return FundHolding.invalid(fundCode: code)
            }
               ...
               let ssgsz = lastData[1] as? String {
                ...
                if let detailUrl = URL(string: detailUrlString) {
                    do {
                        ...
                        await addLog("基金代码 \(code): 获取基金详情失败: \(error.localizedDescription)", type: .error)
                    }
                }
                ...
                if holding.isValid {
                    await addLog("基金代码 \(code): 腾讯财经API解析成功", type: .success)
                } else {
                    await addLog("基金代码 \(code): 腾讯财经API数据无效", type: .error)
                }
                
                return holding
            }
            
            await addLog("基金代码 \(code): 腾讯财经API数据解析失败", type: .error)
            return FundHolding.invalid(fundCode: code)
        } catch {
            await addLog("基金代码 \(code): 腾讯财经API请求失败: \(error.localizedDescription)", type: .error)
            return FundHolding.invalid(fundCode: code)
        }
    }
    ...
    private func fetchFromFund123(code: String) async -> FundHolding {
        ...
        guard let url = URL(string: urlString) else {
            await addLog("基金代码 \(code): 蚂蚁基金API URL无效", type: .error)
            return FundHolding.invalid(fundCode: code)
        }
        ...
        do {
            ...
            let (data, response) = try await URLSession.shared.data(for: request)
            guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
                await addLog("基金代码 \(code): 蚂蚁基金API响应状态码非200", type: .error)
                return FundHolding.invalid(fundCode: code)
            }
            ...
            if let htmlString = String(data: data, encoding: .utf8) {
                ...
                if holding.isValid {
                    await addLog("基金代码 \(code): 蚂蚁基金API解析成功", type: .success)
                } else {
                    await addLog("基金代码 \(code): 蚂蚁基金API数据无效", type: .error)
                }
                
                return holding
            }
            
            await addLog("基金代码 \(code): 蚂蚁基金API数据解析失败", type: .error)
            return FundHolding.invalid(fundCode: code)
        } catch {
            await addLog("基金代码 \(code): 蚂蚁基金API请求失败: \(error.localizedDescription)", type: .error)
            return FundHolding.invalid(fundCode: code)
        }
    }
    ...
    private func fetchFromTHS(code: String) async -> FundHolding {
        ...
        guard let url = URL(string: urlString) else {
            await addLog("基金代码 \(code): 同花顺API URL无效", type: .error)
            return FundHolding.invalid(fundCode: code)
        }
        ...
        do {
            ...
            let (data, response) = try await URLSession.shared.data(for: request)
            guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
                await addLog("基金代码 \(code): 同花顺API响应状态码非200", type: .error)
                return FundHolding.invalid(fundCode: code)
            }
            ...
            if let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
               ...
               let fundData = dataArray.first {
                ...
                if holding.isValid {
                    await addLog("基金代码 \(code): 同花顺API解析成功", type: .success)
                } else {
                    await addLog("基金代码 \(code): 同花顺API数据无效", type: .error)
                }
                
                return holding
            }
            
            await addLog("基金代码 \(code): 同花顺API数据解析失败", type: .error)
            return FundHolding.invalid(fundCode: code)
        } catch {
            await addLog("基金代码 \(code): 同花顺API请求失败: \(error.localizedDescription)", type: .error)
            return FundHolding.invalid(fundCode: code)
        }
    }
    ...
    private func saveToCache(holding: FundHolding) {
        cacheQueue.sync {
            ...
            Task { @MainActor in
                await addLog("基金代码 \(holding.fundCode): 数据已存入主缓存。", type: .cache)
            }
        }
    }
    ...
    private func persistCacheToUserDefaults() {
        do {
            ...
            Task { @MainActor in
                await addLog("持久化主缓存失败: \(error.localizedDescription)", type: .error)
            }
        }
    }

    func addLog(_ message: String, type: LogType) async {
    ...
    func fetchFundDetailsFromEastmoney(code: String) async -> (fundName: String, returns: (navReturn1m: Double?, navReturn3m: Double?, navReturn6m: Double?, navReturn1y: Double?)) {
        await addLog("基金代码 \(code): 尝试从天天基金获取详情数据", type: .network)
        ...
        do {
            ...
            await addLog("基金代码 \(code): 详情数据解析完成: 名称=\(fundName), 1月=\(navReturn1m ?? 0), 3月=\(navReturn3m ?? 0), 6月=\(navReturn6m ?? 0), 1年=\(navReturn1y ?? 0)", type: .success)
            return (fundName: fundName, returns: (navReturn1m, navReturn3m, navReturn6m, navReturn1y))
            
        } catch {
            await addLog("基金代码 \(code): 天天基金详情API请求或正则解析失败: \(error.localizedDescription)", type: .error)
            return (fundName: "N/A", returns: (nil, nil, nil, nil))
        }
    }
